...
let S3Backend =
      { Type =
          ...
          , assume_role_policy_arns : Optional (List Text)
          , assume_role_tags : Optional (Map Text Text)
          , assume_role_transitive_tag_keys : Optional (List Text)
          , workspace_key_prefix : Optional Text
          , force_path_style : Optional Bool
          , max_retries : Optional Integer
          }
      , default =
        { region = env:AWS_REGION as Text ? env:AWS_DEFAULT_REGION as Text ? ""
        ...
        , assume_role_duration_seconds = None Integer
        , assume_role_policy = None Text
        , assume_role_policy_arns = None (List Text)
        , assume_role_tags = None (Map Text Text)
        , assume_role_transitive_tag_keys = None (List Text)
        , workspace_key_prefix = Some "env:"
        , force_path_style = Some False
        , max_retries = Some +5 : Optional Integer
        }
      }
